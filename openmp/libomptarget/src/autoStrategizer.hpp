 ...
 *
 */
enum operation { D2D, BRC, SCT, GAT, RED };
...
public:
  CollectiveOperation(){};
  ~CollectiveOperation(){};
  void add_origin(int a_orig) { o_orig.push_back(a_orig); };
  void add_destination(int a_dest) { o_dest.push_back(a_dest); };
...
public:
  // cops_dep(int a_deps, int a_done, int a_orig, int a_dest, int a_size, int
  // a_oof, int a_dof): deps(a_deps), done(a_done), orig(a_orig), dest(a_dest),
  // size(a_size), of_s(a_oof), of_d(a_dof){};
  OperationDependence(int a_deps, int a_done, int a_orig, int a_dest, int a_oid,
                      int a_did, int a_size, int a_oof, int a_dof, int a_ipth)
      : deps(a_deps), done(a_done), orig(a_orig), dest(a_dest), o_id(a_oid),
        d_id(a_did), size(a_size), of_s(a_oof), of_d(a_dof), ipth(a_ipth){};
  ~OperationDependence(){};
...
public:
   ...
   * Each data movement represents a single D2D move from an origin to a
   * destination of a given size, with a given offset.
   *
   ...
   */
  // depGraph getStrategy(device_id *source, device_id *destination, operation
  // operation, algorithm alg, uint64_t size);
