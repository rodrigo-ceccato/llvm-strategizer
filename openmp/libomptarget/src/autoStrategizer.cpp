...
int get_topo(std::ifstream *arch_f, Architecture *a_arch) {
  int line_n = 0, word_n = 0, n_dev_v = 0, n_net_v = 0, dev_a = 0, dev_b = 0,
      n_core = 0, nma_x = 0;
    ...
    while (!arch_f->eof()) {
      ...
      while (iss_a >> word) {
        if (line_n == 0) {
          ...
          for (dev_b = a_arch->get_nhst();
               dev_b < a_arch->get_ndev() + a_arch->get_nhst(); ++dev_b) {
            iss_a >> word;
            if (word.compare("X") == 0)
              a_arch->get_devmx()[dev_a][dev_b] = 0;
            else if (word.compare("NV1") == 0)
              a_arch->get_devmx()[dev_a][dev_b] =
                  25; // Based on experiments 22 GB/s
            else if (word.compare("NV2") == 0)
              a_arch->get_devmx()[dev_a][dev_b] =
                  50; // Based on experiments 45 GB/s
            else if (word.compare("SYS") == 0)
              a_arch->get_devmx()[dev_a][dev_b] =
                  0; // No P2P - Based on experiments 6 GB/s
            else
              a_arch->get_devmx()[dev_a][dev_b] = -1;
          }
          ...
          while (iss_b >> word_b || n_core == 3) {
            if (std::stringstream(word_b) >> numa_t.h_i[n_core])
              ++n_core;
          }
...
Architecture::Architecture()
    : n_dev(0), n_net(0), n_hst(0), n_nma(0), n_nod{0}, dev_mx(nullptr),
      dev_mx_cpy(nullptr) {
...
AutoStrategizer::AutoStrategizer(std::string architectureFile)
    : dev_a(0), dev_b(0), dev_i(0), dev_ii(0), n_nodes(0), arr_len(0), n_org(0),
      n_dst(0), f_sts(0) {
...
void AutoStrategizer::printCO(printMode mode) {
  if (mode == CLI) {
    ...
    for (auto &it : all_ops) {
      ...
      if (it->get_coop() == D2D)
        printf("D2D");
      else if (it->get_coop() == BRC)
        printf("BRC");
      else if (it->get_coop() == SCT)
        printf("SCT");
      else if (it->get_coop() == GAT)
        printf("GAT");
      else if (it->get_coop() == RED)
        printf("RED");

      printf(", Mthd: ");
      if (it->get_mhtd() == P2P)
        printf("P2P");
      else if (it->get_mhtd() == DVT)
        printf("DVT");
      else if (it->get_mhtd() == MXF)
        printf("MXF");
...
void AutoStrategizer::addCO(CollectiveOperation *a_CO) {
  ...
  // this->ops_deps(&t_arch, &all_ops, &all_deps, t_arch.get_devmxC(),
  // &all_meminfo);
  this->ops_deps();
}

OD_vector *AutoStrategizer::getDeps() { return &all_deps; }

void **AutoStrategizer::get_memptr() { return mem_ptr; }

int AutoStrategizer::get_nnod() { return t_arch.get_nnod(); }
...
// void ops_deps(Architecture * a_arch, CO_vector *all_ops, OD_vector *all_deps,
// int ***dev_mx_cpy_p, MI_vector *all_meminfo)
void AutoStrategizer::ops_deps() {
  int m_paths = 4, m_hops = 5,
      ind_p =
          1; // ToDo: define as inputs max_paths, max_hops, use indirect_paths
  int dev_a = 0, dev_b = 0, dev_i, dev_ii, i_hops, n_hops, i_paths, p_done,
      max_bw, lnk_bw, i_link, n_link, h_aff, f_new;
  float min_lat;
  ...
  public:
    ...
    op_path(int a_n_id, float a_p_bwth, int a_n_hops)
        : n_id(a_n_id), n_hops(a_n_hops) {
      p_op_path = NULL;
      // p_lat = 1/a_p_bwth;
      p_lat = 0;
    };
    // Constructor for link
    op_path(int a_n_id, float a_p_bwth, int a_n_hops, op_path *a_op_path)
        : n_id(a_n_id), n_hops(a_n_hops), p_op_path(a_op_path) {
  ...
  for (auto &t_op : this->all_ops) {
    ...
    case D2D:
        ...
        for (auto &dev_b : *t_op->get_dest()) {
          ...
          if (t_arch.get_devmx_cpy()[dev_a][dev_b] > 0) {
            ...
            printf("[AUST:] Valid H/D Orig: %d Dest: %d Value: %d\n", dev_a,
                   dev_b, t_arch.get_devmx_cpy()[dev_a][dev_b]); // ToBeDeleted
            all_deps.push_back(new OperationDependence(
                0, 0, dev_a, dev_b, t_arch.node_id[dev_a],
                t_arch.node_id[dev_b], t_op->get_size(), 0, 0, 0));
            all_meminfo.push_back(
                new mem_info{dev_a, sizeof(int) * t_op->get_size()});
            all_meminfo.push_back(
                new mem_info{dev_b, sizeof(int) * t_op->get_size()});
          } else if (ind_p) {
      ...
      case MXF:
        ...
        // all_meminfo->push_back(new mem_info{dev_a, sizeof(int) *
        // t_op->get_size()}); all_meminfo->push_back(new mem_info{dev_b,
        // sizeof(int) * t_op->get_size()});
        // // ToDo: check numa affinty
        // // Check direct connection (1 hop)
        // if (t_arch.get_devmx_cpy()[dev_a][dev_b]>0){
        //   all_deps->push_back(new OperationDependence(0, 0, dev_a, dev_b,
        //   a_arch->node_id[dev_a], a_arch->node_id[dev_b], 0, 0, 0, i_paths));
        //   i_paths++;
        //   // Remove link
        //   printf("[AUST:] Valid H/D Orig: %d Dest: %d Value: %d i_paths:
        //   %d\n", dev_a, dev_b, t_arch.get_devmx_cpy()[dev_a][dev_b],
        //   i_paths); //ToBeDeleted t_arch.get_devmx_cpy()[dev_a][dev_b] = 0;
        ...
        //       if (dev_i != dev_b && t_arch.get_devmx_cpy()[dev_a][dev_i] > 0
        //       && t_arch.get_devmx_cpy()[dev_i][dev_b] > 0 ) {
        //         // Get the minimum value of the link
        //         lnk_bw = (t_arch.get_devmx_cpy()[dev_a][dev_i] >
        //         t_arch.get_devmx_cpy()[dev_i][dev_b]) ?
        //         t_arch.get_devmx_cpy()[dev_i][dev_b] :
        //         t_arch.get_devmx_cpy()[dev_a][dev_i];
        //         // Set max bandwidth
        //         if (lnk_bw > max_bw) {
        //           max_bw = lnk_bw;
        //           dev_ii = dev_i;
        ...
        //       op_deps = new OperationDependence(0, 0, dev_a, dev_ii,
        //       a_arch->node_id[dev_a], a_arch->node_id[dev_ii], 0, i_paths, 0,
        //       i_paths); all_deps->push_back(op_deps); op_deps = new
        //       OperationDependence(1, 0, dev_ii, dev_b,
        //       a_arch->node_id[dev_ii], a_arch->node_id[dev_b], 0, 0, i_paths,
        //       i_paths); all_deps->push_back(op_deps);
        //       // Define memory information
        //       all_meminfo->push_back(new mem_info{dev_ii, 0});
        //       // Remove links
        //       printf("[AUST:] Valid H/D Orig: %d Dest: %d MaxBW: %d i_paths:
        ...
        //   c_dep->dest, c_dep->size, c_dep->offs); dev_i++;
        // }

        // for (auto& a_mem : *all_meminfo) {
        //   if (a_mem->node_id != dev_a && a_mem->node_id != dev_b) {

        //     a_mem->size = t_op->get_size()/i_paths * sizeof(int); // ToDo:
        //     MEM integers only
        //   }
        // }
      ...
      case DVT:
        ...
        //   t_arch.get_devmx_cpy()[dev_a][dev_b]); //ToBeDeleted
        //   all_meminfo->push_back(new mem_info{dev_a, sizeof(int) *
        //   t_op->get_size()}); all_meminfo->push_back(new mem_info{dev_b,
        //   sizeof(int) * t_op->get_size()});
        //   // op_deps = new OperationDependence(0, 0, dev_a, dev_b, 0, 0,
        //   i_paths);
        ...
        //       if (dev_i != dev_ii && t_arch.get_devmx_cpy()[dev_i][dev_ii] >
        //       0) {
        //         // Add link to list
        //         printf("Creating II Link %d to %d bw: %d, parent %d\n",
        //         dev_i, dev_ii, t_arch.get_devmx_cpy()[dev_i][dev_ii],
        //         (v_paths.at(i_link-1))->n_id); iop_path = new op_path(dev_i,
        //         static_cast< float >(t_arch.get_devmx_cpy()[dev_i][dev_ii]),
        //         i_hops, v_paths.at(i_link-1)); v_paths.push_back(iop_path);
        ...
        //           v_paths.at(i_link)->p_lat); vi_paths.push_back(iop_path);
        //         }
        //       }
        //       // printf("Testing H/D Orig: %d Dest: %d Value: %d\n", dev_i,
        //       dev_b, t_arch.get_devmx_cpy()[dev_i][dev_b]); //ToBeDeleted
        //     }
        //     if (n_link > i_link) {
        ...
        //   for (auto i_path = vi_paths.begin(); i_path != vi_paths.end();
        //   i_path++) {
        //     if (min_lat > (*i_path)->p_lat) {
        //       printf("Path to %d latency: %f\n", (*i_path)->n_id,
        //       (*i_path)->p_lat); min_lat = (*i_path)->p_lat; it_path =
        //       i_path; iop_path = (*i_path);
        //     }
        //   }
        //   if (min_lat != 1000) {
        ...
        //       all_deps->push_back(new OperationDependence(iop_path->n_hops-1,
        //       0, iop_path->n_id, iop_path->o_id,
        //       a_arch->node_id[iop_path->n_id],
        //       a_arch->node_id[iop_path->o_id], 0, 0, 0, i_paths));
        //       // Define memory information
        //       printf("[min] Path from %d to %d latency: %f\n",
        //       iop_path->n_id, iop_path->o_id, iop_path->p_lat); if
        //       (iop_path->n_id != dev_a) all_meminfo->push_back(new
        //       mem_info{iop_path->n_id, 0}); iop_path = iop_path->p_op_path;
        //     }
        //     i_paths++;
    ...
    case BRC:
      ...
      //   all_meminfo->push_back(new mem_info{dev_b, sizeof(int) *
      //   t_op->get_size()});
      ...
      case P2P:
        ...
        //     dev_a = *(t_op->get_orig())->begin();
        //     // all_meminfo->push_back(new mem_info{dev_a, sizeof(int) *
        //     t_op->get_size()}); for (auto dev_b : *t_op->get_dest()) {
        //       // Check direct path P2P
        //       if (t_arch.get_devmx_cpy()[dev_a][dev_b]>0) {
        //         // ToDo: Remove available link?
        //         printf("Valid H/D Orig: %d Dest: %d Value: %d\n", dev_a,
        //         dev_b, t_arch.get_devmx_cpy()[dev_a][dev_b]);
        //         all_deps->push_back(new OperationDependence(0, 0, dev_a,
        //         dev_b, a_arch->node_id[dev_a], a_arch->node_id[dev_b],
        //         t_op->get_size(), 0, 0, 0));
        //         // all_meminfo->push_back(new mem_info{dev_b, sizeof(int) *
        //         t_op->get_size()});
        //       } else if (ind_p){
      ...
      case MXF:
        ...
        //       all_deps->push_back(new OperationDependence(0, 0, dev_i,
        //       dev_ii, a_arch->node_id[dev_i], a_arch->node_id[dev_ii],
        //       t_op->get_size(), 0, 0, i_paths));
        //     } else {
        //       all_deps->push_back(new OperationDependence(1, 0, dev_i,
        //       dev_ii, a_arch->node_id[dev_i], a_arch->node_id[dev_ii],
        //       t_op->get_size(), 0, 0, i_paths));
        //     }
        //     // Remove links
        ...
        //     t_arch.get_devmx_cpy()[dev_i][dev_ii] = 0;
        //     t_arch.get_devmx_cpy()[dev_ii][dev_i] = 0;
        //     op_dest.erase(std::remove(op_dest.begin(), op_dest.end(),
        //     dev_ii), op_dest.end()); op_orig.push_back(dev_ii); i_paths++;
        //   } else {
      ...
      case DVT:
        ...
        //       printf("Creating II Link %d to %d bw: %d, parent %d\n", dev_i,
        //       dev_i, t_arch.get_devmx_cpy()[dev_i][dev_ii],
        //       (v_paths.at(i_link-1))->n_id); iop_path = new op_path(dev_i,
        //       static_cast<float>(t_arch.get_devmx_cpy()[dev_i][dev_ii]),
        //       i_hops, v_paths.at(i_link-1)); v_paths.push_back(iop_path);
        //       t_arch.get_devmx_cpy()[dev_i][dev_ii] = 0;
        ...
        //             if ((*a_path)->n_id == dev_ii && (*a_path)->p_lat >
        //             iop_path->p_lat) {
        //               printf("Lower latency - Existing destination %d\n",
        //               dev_ii); printf("Valid Link %d to %d latency: %f\n",
        //               dev_i, dev_b, v_paths.at(i_link)->p_lat); a_path =
        //               vi_paths.erase(a_path); vi_paths.push_back(iop_path);
        //               f_new = 0;
        //             } else {
        //               a_path++;
        //             }
